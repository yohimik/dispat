...
| [`ci-worker.sh`](./ci-worker.sh)           | the release's full-suite job                           | `DISPAT_EXECUTION_SECRET`, `DISPAT_CI_WORKER_TOKEN` (a GitHub App token minted for the run), `DISPAT_CI_WORKER_*`, the Workload Identity credentials | One Compute Engine worker per run: created, started as `dispat worker` polling this repository over https with its own token, its log collected, deleted, and the run's leftover `dispat-worker-*` branches pruned; the job runs the suite against it with `--worker <name>`, so the repository itself is the mailbox. |
