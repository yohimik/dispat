...
run_experiment() {
  ...
  step query:status dispat status --require-release --log-format json
  echo "   next plan: $(json_lines "$OUT/step-status.log" | jq -r 'select(has("bump")) | .package + " " + .version' | tr '\n' ';')"
  ...
  assert "the next plan is empty" plan_is_empty status
}
