...
run_experiment() {
  ...
  step release:version nx release --skip-publish
  ...
  allow cli
  catch_up
  step release:recovery nx release publish --registry "$REGISTRY"
  ...
  assert "the next plan is empty" plan_is_empty next
}
