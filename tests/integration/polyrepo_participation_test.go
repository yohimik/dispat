...
// TestPolyrepoExclusionTakesTheDeclarationsItOwns: a space path inside an
...
func TestPolyrepoExclusionTakesTheDeclarationsItOwns(t *testing.T) {
	control := participationFleet(t)
	...
	assert.Contains(t, combinedOutput(res), "repository excluded from the release")
	assert.Contains(t, combinedOutput(res), "archive-source")
	assert.Contains(t, combinedOutput(res), "legacy-source")
}

// TestPolyrepoExcludedDependencyEndpointsSayWhatIsMissing: an endpoint that
...
func TestPolyrepoExcludedDependencyEndpointsSayWhatIsMissing(t *testing.T) {
	...
	t.Run("an endpoint no exclusion accounts for", func(t *testing.T) {
		control := participationFleet(t)
		cfg := polyrepoModelFile()
		cfg.Spaces = polyrepoModelSpaces(map[string]string{"kept": "sources/kept/packages"})
...
// participationFleet links two sources that hold packages and one
...
func participationFleet(t *testing.T) *harness.Repo {
	t.Helper()
	kept := harness.New(t)
