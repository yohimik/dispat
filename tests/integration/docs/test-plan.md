...
| `TestCovTailSelfUpdateRefusesAnAnswerThatIsNotARelease` | Each way somebody else's server can fail to answer with a release is refused on its own terms: a listing that is not a listing, a name that is not a version, and a lookup body that is not a release. |
