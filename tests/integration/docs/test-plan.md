...
16. **Release records** (`records_test.go`): the durable artefacts themselves: changelog files accumulating across
    ...
    take it back: a tag (E210), a tag at a foreign commit (E211), a record (E212), the release commit (E213) and the
    push (E214) each failing there, plus the alias tag (W232) that deliberately is not one of them. None of them
...
21. **The shell helpers** (`if_test.go`, `exec_test.go`): the two commands that run one script instead of sweeping a
    ...
    load-bearing claim is that a declared script reading `DISPAT_*` becomes runnable outside a release.
...
Three subjects are asserted from more than one goal, on purpose, because a property and the feature that carries it
...
| `TestRecordsPushVerifyDisabled`                                   | `commit.verify=false` switches the upfront ls-remote check off: the release work happens and only the push itself fails (E214); asserted against the default in the same test, which fails fast before any work (no tags, no changelog).                                                                                                                              |
...
| `TestRecordsTagAtAnotherCommitIsLeftAlone` | A tag dispat can see at the wrong commit is reported (E211) and left there, because a tag moved here would be force-pushed over the copy on the remote and turn one local mistake into everyone's. Reached through `dispat commit --tag-name`, since a release plans its version *from* the tags. |
