...
| `TestReleaseRefusesToRepublishAnExistingTag` | The recovery pushes this run's tags again and `commit.force` is on by default, so a checkout stale enough to have re-planned an already published version would force-move a released tag. The remote's tags are read first: the run stops, names the tag, and the published ref is left where its own release put it. |
