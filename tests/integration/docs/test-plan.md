...
| `TestPlanFailedProviderSkipsTrainConsumer`                | The skip cascade on a train: with its own work published train history and its only fresh cause the failing provider's propagation, the consumer is skipped rather than released — a prerelease whose entire content is a movement that never published would record a version that does not exist. The healing run releases both as ordinary propagation, no W193. |
...
| `TestAutoWriterRedirectsThroughTheBinary`                      | `--link` adds the local-folder directive across the selection and an empty path restores the declared range.                                                                                                                       |
