...
| `TestExecutionOrchestratorGitFaults` | The git invocations the orchestrator issues past preflight, failed one at a time and read off a trace of a real distributed run. A message it can never resolve, measure or read refuses the release with E225 before a stage runs anywhere; an input state it cannot commit or offer, an assignment it cannot describe or offer and a preparation it cannot offer each fail their package with E227 and publish nothing; and a fetch or a chain walk of a node's answer that fails once costs a tick, after which the release is still a release and its branches are closed. |
| `TestExecutionWorkerTransportGitFaults` | The same sweep on the node: input states it cannot fetch, a package folder it cannot resolve inside the state it was given and a claim it cannot write each fail the task with E227 and publish nothing, while the administrative record of a checkout it cannot prune is W244 on the node and the release still writes its tags. The node carries on serving in every row. |
