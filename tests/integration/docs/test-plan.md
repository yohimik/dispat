...
| `TestAdmissionNestedTagCatchesUpAfterTheProviderShipsAlone` | The same debt with the consumer's publish flow invoking native `dispat commit --tag` in commit mode. The outer run records a changelog commit of its own, which moves the head past the consumer's tag, so the provider ships alone, the consumer catches up once and the plan empties. |
