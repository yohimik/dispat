...
| `TestExecutionRunOutputsMergeFailureFailsTheSweep` | A set whose second file would replace a folder in the orchestrator's checkout is refused with E227 `destination-component`. Its first file is restored to its previous bytes, the blocking folder and temporary names are left alone, another package's set still merges, and the sweep exits 1. |
