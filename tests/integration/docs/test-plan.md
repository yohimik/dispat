...
| `tests/integration/execution_ownership_test.go::TestExecutionLostLockIsRememberedForEveryLaterTask` | The first of three independent builds takes the release lock off the remote and then holds the only node for longer than the ownership answer is cached: the next task asks the remote, finds the lock gone and decides the loss, and the one after it is refused from that decision. The loss is reported exactly once however many tasks it stops, exactly one build ran anywhere, nothing is recorded, no publication is authorized, and the lock the run lost is neither re-created nor deleted by it. |
...
| `TestExecutionRunOutputsMergeFailureFailsTheSweep` | A set whose file would replace a folder in the orchestrator's checkout is refused with E227 `destination-component` and merged not at all, leaving the folder as it was, the other sets are still merged, and the sweep exits 1. |
