...
| `TestAdmissionCatchesUpAfterTheProviderShipsAlone` | Three separate runs: the consumer publishes its own change against the failed provider's baseline, the provider publishes alone with the consumer filtered out, then an unfiltered run at the same commit publishes only the consumer catch-up. The consumer's immutable release tag records the exact provider tag it saw, so equal commit targets cannot silently discharge the debt. The range moves, W193 explains the catch-up, and a later status is empty. |
| `TestUnicodeDependencyReceiptRemainsReadableOnTheNextRun` | A dependency names its provider through a Unicode case alias; release records the provider's exact tag spelling, and the next run reads the receipt and converges without publishing either package again. |
| `TestAdmissionRemovedProviderLeavesHistoricalReceiptInert` | In one repository, a consumer ships its own change while its provider publish fails; after the provider package and dependency are deliberately removed, the consumer's immutable receipt remains historical evidence and status converges without inventing a new dependency or release. |
| `TestAdmissionNestedTagRetainsProviderReceipt` | The same three-run debt with the consumer's publish flow invoking native `dispat commit --tag`. That nested command replans under the outer run, yet its release tag preserves the provider observation made by the outer scheduler; a later provider-only publish still leaves exactly one consumer catch-up and never repeats the provider. |
| `TestAdmissionOversizedProviderReceiptRefusesBeforePublish` | A fan-in of 90 long-named providers would produce a receipt above the bounded publish environment size. Release refuses at plan preflight with no package publish marker and no tag, before any provider can partially publish. |
| `TestAdmissionNestedReceiptUnknownProviderRefusesBeforeCommit` | A nested publish context supplies either a canonical receipt naming a package absent from the composed plan or one above the generated-size limit. `commit --tag` refuses both before making a release commit or tag, retaining the original HEAD. |
| `TestAdmissionNestedOtherPackageKeepsItsOwnReceipt` | A consumer's publish script explicitly invokes native `dispat commit --tag --package util`. The nested command tags the unrelated package from its own replan, with an empty provider receipt, instead of inheriting the outer consumer's core observation. |
| `TestAdmissionCatchesUpFromAnUnreleasedProvider` | The consumer first publishes before the provider has any tag, then ships own work while the provider fails. The provider's first release arrives in a filtered run; an unfiltered run reads the consumer's empty observed-provider boundary and publishes its catch-up once. |
| `TestAdmissionReceiptKeepsTheExactProviderTag` | The provider's parseable baseline tag includes SemVer build metadata. A consumer release while that provider fails records the exact tag ref rather than a rendered version that would drop the metadata; provider-only retry and later catch-up both succeed without an invented tag. |
| `TestAdmissionRefusesDamagedDeliveryEvidence` | An annotated consumer tag with a malformed provider receipt, or a receipt naming a provider tag later deleted, makes status fail rather than silently reinterpret the release by ancestry and erase an owed pickup. |
| `TestAdmissionFinalTagFailureCannotForgeDelivery` | In deferred release-commit mode both publish scripts succeed, then a late collision makes the provider's final tag fail. The consumer's tag is blocked because it would claim the missing provider tag in its immutable receipt; the run reports a critical failure. |
| `TestAdmissionCancelAfterProviderShipsAlone` | A consumer's new `cancel(cli)` discards an owed pickup after the provider-only run. Its earlier release tag contains the source commit but did not receive the provider version, so the cancel reaches the delayed contribution and the plan becomes empty. |
...
| `TestRecordsTagsAreAnnotatedWithReleaseMessages`                  | A release tag is an annotated tag *object* (`cat-file -t` = `tag`), its message carries `release <tag>` and a canonical empty provider receipt, and it peels to the commit that was released.                                                                                                                                                                                                              |
...
| `services/dispat/internal/gitx/tag_message_test.go::TestCreateTagStreamsLargeAnnotation` | A large annotated release tag preserves its complete provider-receipt body through the real Git tag writer, so publication evidence is not truncated by process arguments. |
...
| `TestChoreographyMissingProviderTagBlocksConsumerReceipt` | A source repository's commit hook creates a conflicting release tag only after planning and publish. The source record fails, the consumer is W194-blocked, and no consumer tag can claim delivery of the missing provider record. |
| `TestChoreographyCatchesUpAfterProviderOnlyRetry` | In linked repositories the app first publishes its own work while the library publish fails, the library succeeds in a later filtered run, and the app catches up in a third run with no new source commit. The library has exactly one new tag, the app's catch-up is W193, and fleet-link history supplies the cross-repository boundary. A later deletion of the exact foreign tag named by the app's receipt fails status rather than inventing a boundary. |
| `TestChoreographyRemovedProviderLeavesHistoricalReceiptInert` | A linked consumer tag records a propagated provider release, then the provider package and its dependency are retired while the repository remains in the fleet. Status still plans the surviving consumer without treating its immutable historical receipt as a current dependency or inventing another release. |
...
| `TestFinalPolyrepoMalformedTagInventoryCannotEraseThePublishedBaseline` | Missing ref fields, including the receipt-bearing subject, a valid row followed by a malformed row, missing tag names, and invalid object or peeled identities cannot erase or move a published tag; a healthy retry plans the pending fix. |
