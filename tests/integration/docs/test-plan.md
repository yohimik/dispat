...
2. **Space versioning modes** (`versioning_test.go`): all seven modes driven side by side across multiple runs:
   ...
   catch-up, holds and pins under a shared version, mixed shared depths in one group, and no bleed between modes.
...
16. **Release records** (`records_test.go`): the durable artefacts themselves: changelog files accumulating across
    ...
    and commit mode's release commit, tag placement and push against a real bare remote, including what happens when
    a tag or a push fails after the release is already out.
...
| `TestRecordsAliasTags` | The alias feature end to end beside a path-prefixed release tag: the moving ref follows the newest stable release, the exact one is written per release and never moves, and a prerelease writes its own exact ref while leaving the moving one where it is. |
