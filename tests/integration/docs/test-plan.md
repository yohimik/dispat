...
| `TestExecutionLostAuthorizationResponseRetainsExclusion` | An authorization push applies and a real worker starts its publish command before the response is lost. Whether the branch remains, disappears or is reset to its previous state, the run reports E228, retains the release lock, records no successful version and starts no second publication. |
