...
| `TestExecutionSecretNeverReachesMailboxOrLogs` | The signing secret stays on the machines that hold it, in all three history modes: it is in neither the orchestrator's output nor any node's, and in none of the blobs the mailbox holds, which are read object by object because a finished run deletes the branches that pointed at them. |
