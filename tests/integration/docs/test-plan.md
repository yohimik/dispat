...
4. **Everything else** (`config_test.go`): config validation and precedence, config file discovery, login scripts,
   the `run.onFail` / `run.onSkip` outcome scripts, GitHub release assets from build-exported attachments, and
...
Shapes the model deliberately cannot express (an unknown key, the legacy flat space schema) are written as
`map[string]any` through `harness.WriteConfigRaw`, because the suite pins that migration edge: a config still using
the legacy flat keys (`buildScript` on a space) must be *rejected* at load, not silently ignored into a script-less
release.
