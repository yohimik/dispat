...
| `TestSelfUpdateRefusesAnUnsafePreviousBackup` | A directory owned by another process at the backup path blocks the second install before the current executable moves; both its bytes and the directory's contents remain intact. |
