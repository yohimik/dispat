...
| `tests/integration/execution_nodestate_test.go::TestExecutionWorkerStateRefusals` | The state folder unusable in each of the ways a file system can make it so, one row each: a node folder whose name something else holds, a lock that cannot be read, an answered-work record that is not JSON and one that cannot be read. None of them starts a node, each says why it cannot serve, and nothing of the mailbox is read. |
