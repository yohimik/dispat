...
| `TestExecutionAuthorizedWithdrawalNeedsItsOwnAcknowledgement` | After Go and Cancel, an authentic acknowledgement that the command never started makes the outcome known and releases the lock; an acknowledgement naming an earlier cancellation, another work kind or signed with another key cannot establish quiescence, so the outcome stays unknown and its lock and branch remain. |
