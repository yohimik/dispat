...
| `TestExecutionWorkerConcurrentStaleStateClaimHasOneOwner` | Four separate worker processes contend for one state folder whose lock contains a stale PID; exactly one serves, the others refuse, and the lock inode stays the same after every exit. |
...
| **A stale worker state lock could admit two serving processes.** Renaming a stale PID file aside created an absent-path window: a second claimant could create a different lock inode while the first owner kept serving. The worker now holds a kernel lock on one stable inode for its lifetime, never renames or removes it, and preserves a live legacy PID refusal. | `TestExecutionWorkerConcurrentStaleStateClaimHasOneOwner`, `TestExecutionWorkerStartRefusals` | `execution_worker_test.go`; `internal/execution` `TestNodeStateLockKeepsOneInodeAcrossHolders` |
