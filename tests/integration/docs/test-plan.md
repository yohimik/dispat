...
| `TestExecutionChangedProviderWithholdsDependentPublications` | A commit made after a provider publishes, inside that provider's folder, invalidates the artefacts of downstream packages built from it even when their own folders stay unchanged: each dependent publication is withheld with E227 before its command runs, while the provider's release remains tagged. |
