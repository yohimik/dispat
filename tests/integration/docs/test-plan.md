...
| `TestRecordsPushNeverReplacesARecordTheRemoteHolds` | A record the remote holds off this checkout's history changes no plan, so the run goes ahead and the push decides: the name is created or what is there is left alone. The remote keeps its record, the package stays `published`, and the run reports E221 and exits non-zero instead of force-moving a published ref. |
