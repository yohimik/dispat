...
| `TestExecutionStaleReceiptIsRejected` | Every identity a receipt is bound to, wrong in turn: a signed reply of another run, another planning, another attempt, another ownership generation, another assignment object, another node, another branch, and one signed with a secret this run does not hold, are each ignored for exactly one named reason and grant the attempt nothing, which fails on its own deadline with no tag; the same saboteur bound correctly releases the package. |
| `TestExecutionForgedClaimsAreRejectedBeforeWork` | A mailbox writer sends a claim that is signed by another secret, cannot be parsed as a claim, has an expired issue time, or answers another assignment. The coordinator rejects each for its own reason, sees no work as completed and publishes no version. Cancellation and cleanup preserve the exact rejected tip. |
| `TestExecutionForgedReadyCannotAuthorizePublication` | After a valid claim, a mailbox writer sends a publish-ready message signed by another secret, with an unparseable header, or naming another claim. The coordinator refuses each with its own reason and sends no authorization: only the local build runs, no publish command runs and no version is tagged. Cancellation and cleanup preserve the exact rejected tip. |
...
| `TestExecutionAuthorizedWithdrawalNeedsItsOwnAcknowledgement` | After Go and Cancel, an authentic acknowledgement that the command never started makes the outcome known and releases the lock; an acknowledgement naming an earlier cancellation or signed with another key cannot establish quiescence, so the outcome stays unknown and its lock and branch remain. |
...
| `tests/integration/execution_authorization_test.go::TestExecutionCraftedPublicationAuthorizations` | Every rule a publishing node holds an authorization to, offered as a message written by hand while a real node waits at the gate: another secret, no signature, a signature over another kind of message, a document that is not JSON, an expiry in the past, no expiry at all, another node, another branch, another protocol, another ready, another assignment, another run, another attempt, another ownership and another plan. Each is refused with the rule it broke named and nothing it contained echoed, no publish command runs for any of them although every one of them had run its `beforePublish` hook, and no refused attempt answers on a branch somebody else had moved. Every refusal is a known outcome the node reports where the run can read it: a failed result naming the authorization gate, written on top of the message it refused, so the run is never left to wait out a deadline and to call an authorization it may have written an outcome it cannot establish. |
