...
| `tests/integration/execution_authorization_test.go::TestExecutionCraftedPublicationAuthorizations` | Every rule a publishing node holds an authorization to, offered as a message written by hand while a real node waits at the gate: another secret, no signature, a signature over another kind of message, a document that is not JSON, an expiry in the past, no expiry at all, another node, another branch, another protocol, another ready, another assignment, another run, another attempt, another ownership and another plan. Each is refused with the rule it broke named and nothing it contained echoed, no publish command runs for any of them although every one of them had run its `beforePublish` hook, and no refused attempt answers on a branch somebody else had moved. |
...
| `TestExecutionCraftedPublicationWithdrawals` | A withdrawal is held to the same rules as an authorization, because one nobody signed would let anybody who can write into a mailbox stop every publication: a valid one is acknowledged as the attempt's terminal message with the authorization-wait phase and no command started, one nobody signed and one naming an earlier tip are both refused with the rule named and acknowledged by nobody, and an authentic message of another kind withholds the publication instead of starting it. |
