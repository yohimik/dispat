...
| `tests/integration/execution_recovery_test.go::TestExecutionStaleReceiptIsRejected` | Every identity a receipt is bound to, wrong in turn: a signed reply of another run, another planning, another attempt, another ownership generation, another assignment object, another node, another branch, and one signed with a secret this run does not hold, are each ignored for exactly one named reason and grant the attempt nothing, which fails on its own deadline with no tag; the same saboteur bound correctly releases the package. |
| `tests/integration/execution_recovery_test.go::TestExecutionOldRunReceiptGrantsNothing` | A complete, correctly signed receipt of an earlier run replayed onto this run's branch satisfies nothing: the reply is refused as a replay, no build runs anywhere on the strength of it, no version is tagged and the run fails on the deadline of an attempt nobody answered. |
| `tests/integration/execution_recovery_test.go::TestExecutionLostPublishReplyIsNeverRepublished` | A node killed outright after its publish command ran and before it could report: the package fails at publish with E228 and no release record, the publish command ran exactly once, the run exits non-zero, the exclusion covering that repository is left on its remote with a remedy naming the order of recovery and the release-lock page, and the summary counts the outcome as unknown rather than as published or failed. |
| `tests/integration/execution_recovery_test.go::TestExecutionLostResultPushIsRecognizedNotRepeated` | A node's result push that lands and whose answer is lost is recognised by re-reading the branch: one result answers one build, the build script ran exactly once, the package is released and nothing is reported as an unknown publication outcome. |
| `tests/integration/execution_recovery_test.go::TestExecutionLockLossStopsNewEffects` | The remote release lock deleted while the run's build executes stops every later effect: the publication is withheld with E336, no publish command runs anywhere, no version is recorded, and the lock the run lost is neither re-created nor deleted by it. |
| `tests/integration/execution_recovery_test.go::TestExecutionInterruptCancelsRemoteTasks` | A release interrupted while a node is building withdraws the attempt it has in flight, re-reading the branch when its lease is refused, and waits to be told that nothing of it is running: the node kills the process group, acknowledges with the phase the frame had reached, the package is cancelled rather than failed so no failure hook runs, nothing is recorded, and the node is still serving afterwards. |
| `tests/integration/execution_recovery_test.go::TestExecutionKilledWorkerBlocksDependentsAndKeepsCapacity` | The only node able to run a plan's builds killed mid-build: the package fails with E227, the node's slot stays held and the node leaves the pool with the task-deadline reason, no task of the run completes its computation, no build of a dependent is placed anywhere, nothing is published, no publication outcome is in doubt because none was authorized, and the exclusions are still released because a compute attempt is fenced by revoking its ref. |
| `tests/integration/execution_recovery_test.go::TestExecutionCleanupFailureKeepsPublishedOutcome` | A run whose batched deletion of its own coordination branches cannot be pushed still publishes and tags its package, names the refs that remain in a warning under the retained code and the transport-cleanup category, leaves those refs on the mailbox, and exits zero. |
| `tests/integration/execution_recovery_test.go::TestExecutionTaskReceiptDischargesNothing` | A build that succeeded on a node and a publication that failed leave no release record: the next plan still owes the same package at the same version, and no tag was derived from the receipt for a completed task. |
| `tests/integration/execution_recovery_test.go::TestExecutionPrepublicationFailureRemainsRebuildable` | After a run that failed before publication, deleting the node's whole state folder, the orchestrator's private fetched refs and every branch left on the mailbox changes nothing: the same version is still owed, and the next run rebuilds it on a node whose cache was thrown away and releases it. |
| `tests/integration/execution_recovery_test.go::TestExecutionDeletedTransportStateLeavesRecordsAuthoritative` | After a completed run, deleting the node's state folder, the private fetched refs and every coordination branch and replanning the unchanged inputs proposes no release at all: the durable source record is what discharged the debt. |
