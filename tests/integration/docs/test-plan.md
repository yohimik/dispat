...
| `TestRecordsPushRefusesEveryPackageWhenOneIsAlreadyRecorded`      | A record the remote holds and this checkout does not stops the whole run before anything is built, whatever `commit.force` says: the package with nothing wrong with it is not released either, no release commit is made, and the remote record keeps its original target.                                                                                     |
...
| `TestGitPushRefusesAPlanTheRemoteAlreadyRecorded` | A checkout planning a version the remote has already recorded is refused with E196 before anything runs, under `commit.force` on and off alike, and the remote's annotated record is read peeled and left where it was. Reaching the push and skipping or force-replacing the tag there, which is what this used to assert, published the version a second time either way. |
