...
| `TestExecutionWorkerPrunesAnsweredWorkWhileServing` | Seed an answered-work timestamp just inside the 48-hour retention horizon, run one worker long enough for the boundary to pass, and offer another probe. The first replay is refused while live; the next answer removes the expired tuple from memory and `seen.json` without restarting, while preserving the new tuple. |
