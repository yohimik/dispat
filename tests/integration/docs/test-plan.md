...
| `TestCovPolyrepoRefusesADeclaredSourceItCannotUse` | Every part of a submodule declaration is checked before it may own a package: a path above the control root, a path that is a folder of the control repository, a repository the control HEAD does not pin, and one with no HEAD of its own are each refused naming the declared identity. |
...
| `TestCovPolyrepoImportListComposesFromEverySpelling` | The same two imports reach the same two repositories written as a list or as one value carrying both, and a value naming no file is refused with `E330`. |
...
| `TestCovPolyrepoBeforeCommitHookCannotReplaceThePlannedSource` | A source beforeCommit hook that moves HEAD after publication cannot cause the recorder to tag or push an unplanned revision or checkpoint its gitlink; after explicit repair, the release succeeds. |
