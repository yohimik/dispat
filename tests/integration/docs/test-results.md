...
The suite originally turned up two behaviours contradicting the documentation (a rejected `Release-As` pin swallowing
a sibling bump; a propagated graduation transition never graduating dependants). Both are now **fixed in the
planner**, and the fences were flipped into tests guarding the corrected behaviour. See the
