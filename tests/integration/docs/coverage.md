...
Statement coverage of the workspace's **unit test layer**, measured the way CI computes the badge on the main README:
one `go test -covermode=atomic` profile per module — with `-coverpkg=./...` for the CLI module, so its in-process
end-to-end suites (`cli`, `app`) also count toward the internal packages they drive — merged into a single profile.
The badge always reflects the latest `main` build; the table below is a local snapshot, regenerated on **2026-08-07**
with Go 1.26.
...
The integration suite in this module is deliberately **black-box**: it compiles the real dispat binary and drives it
from the outside, so it produces no statement-coverage profile of dispat's sources on top of the numbers above. What
it covers is behaviour — git state, JSON log events, execution timelines — and its status and per-area summary live
in [test results](./test-results.md), with the claim-by-claim matrix in the [test plan](./test-plan.md).
