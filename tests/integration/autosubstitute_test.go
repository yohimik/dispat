...
// Area 20: the autosubstitute command through the compiled binary. `dispat
// autosubstitute` is `dispat replacer` pointed at a selection instead of a list
...
// asRepo is the fixture: core and web, web depending on core, each with a
...
func asRepo(t *testing.T) *harness.Repo {
	t.Helper()
	r := harness.New(t)
...
func asRead(t *testing.T, r *harness.Repo, parts ...string) string {
	t.Helper()
...
// TestAutoSubstituteFansOutAcrossWorkspaceProviders: one pattern, rendered once
...
func TestAutoSubstituteFansOutAcrossWorkspaceProviders(t *testing.T) {
	r := asRepo(t)

	res := r.Command("autosubstitute", "--since", "all", "--files", "*.gradle",
		"--sub", "com.acme:{provider}:{providerPrevious}=>com.acme:{provider}:{providerVersion}")
	require.Equal(t, 0, res.Code, "stderr:\n%s", res.Stderr)
	assert.Contains(t, asRead(t, r, "packages", "web", "build.gradle"),
		"'com.acme:core:0.1.0'", "the coordinate followed the provider")
}

// TestAutoSubstitutePackageScopedPatternRunsOnce: a --sub naming no provider is
// about the covered package itself, so it is rendered once and --only-updated
// leaves it alone.
func TestAutoSubstitutePackageScopedPatternRunsOnce(t *testing.T) {
	r := asRepo(t)

	res := r.Command("autosubstitute", "--since", "all", "--files", "README.md",
		"--sub", "{name} {previous}=>{name} {version}")
	require.Equal(t, 0, res.Code, "stderr:\n%s", res.Stderr)
	assert.Contains(t, asRead(t, r, "packages", "web", "README.md"), "web 0.1.0",
		"the package's own version was written")
}

// TestAutoSubstituteGlobsSelectWithinThePackage: a glob reaches only what it
// names, and only inside the package folder the sweep handed over.
func TestAutoSubstituteGlobsSelectWithinThePackage(t *testing.T) {
	r := asRepo(t)
	before := asRead(t, r, "packages", "web", "README.md")

	res := r.Command("autosubstitute", "--since", "all", "--files", "*.gradle",
		"--sub", "0.0.0=>9.9.9")
	require.Equal(t, 0, res.Code, "stderr:\n%s", res.Stderr)
	assert.Contains(t, asRead(t, r, "packages", "web", "build.gradle"), "9.9.9")
	assert.Equal(t, before, asRead(t, r, "packages", "web", "README.md"),
		"a file no glob selected is untouched")
}

// TestAutoSubstituteOnlyUpdatedNarrowsTheFanOut: with the providers of this run
// alone, a provider released earlier expands into nothing.
func TestAutoSubstituteOnlyUpdatedNarrowsTheFanOut(t *testing.T) {
	r := asRepo(t)
	...
	res := r.Command("autosubstitute", "--only-updated", "--files", "*.gradle",
		"--sub", "com.acme:{provider}:{providerPrevious}=>com.acme:{provider}:{providerVersion}")
	require.Equal(t, 0, res.Code, "stderr:\n%s", res.Stderr)
	assert.Contains(t, asRead(t, r, "packages", "web", "build.gradle"), "0.0.0",
		"core is not in this run, so its coordinate is left alone")
}

// TestAutoSubstituteConsumersReachesThePackagesTheWindowLeftOut: the package
...
func TestAutoSubstituteConsumersReachesThePackagesTheWindowLeftOut(t *testing.T) {
	r := asRepo(t)
	sub := "com.acme:{provider}:{providerPrevious}=>com.acme:{provider}:{providerVersion}"

	// Bring the coordinate up to date, then release, so the next run has a
	// genuine one-version gap to close.
	require.Equal(t, 0, r.Command("autosubstitute", "--since", "all", "--files", "*.gradle", "--sub", sub).Code)
	assert.Contains(t, asRead(t, r, "packages", "web", "build.gradle"), "com.acme:core:0.1.0")
	r.Command("commit", "--tag")
	...
	res := r.Command("autosubstitute", "--files", "*.gradle", "--sub", sub)
	require.Equal(t, 0, res.Code, "stderr:\n%s", res.Stderr)
	assert.Contains(t, asRead(t, r, "packages", "web", "build.gradle"), "com.acme:core:0.1.0",
		"web is outside the window")

	res = r.Command("autosubstitute", "--consumers", "--files", "*.gradle", "--sub", sub)
	require.Equal(t, 0, res.Code, "stderr:\n%s", res.Stderr)
	assert.Contains(t, asRead(t, r, "packages", "web", "build.gradle"), "com.acme:core:0.2.0",
		"--consumers pulled web in and its coordinate followed")
}

// TestAutoSubstituteConvergesUnderStrict: the probe is what tells "already
...
func TestAutoSubstituteConvergesUnderStrict(t *testing.T) {
	r := asRepo(t)
	args := []string{"autosubstitute", "--since", "all", "--strict", "--files", "README.md",
		"--sub", "{name} {previous}=>{name} {version}"}

	res := r.Command(args...)
	require.Equal(t, 0, res.Code, "stderr:\n%s", res.Stderr)
	first := asRead(t, r, "packages", "web", "README.md")

	res = r.Command(args...)
	assert.Equal(t, 0, res.Code, "a converged re-run is clean, not stale; stdout:\n%s", res.Stdout)
	assert.Equal(t, first, asRead(t, r, "packages", "web", "README.md"),
		"and it changed nothing")
}

// TestAutoSubstituteOutcomesReachTheExitCode: every way this command can
// refuse, over the process boundary.
func TestAutoSubstituteOutcomesReachTheExitCode(t *testing.T) {
	r := asRepo(t)

	res := r.Command("autosubstitute", "--since", "all", "--strict", "--files", "*.gradle",
	...
	assert.Equal(t, 2, r.Command("autosubstitute").Code, "nothing to write")
	assert.Equal(t, 2, r.Command("autosubstitute", "--sub", "a=>b").Code,
		"a sub with no files to look in selects nothing")
	assert.Equal(t, 2, r.Command("autosubstitute", "--files", "*.md").Code,
		"files with nothing to write")
	assert.Equal(t, 2, r.Command("autosubstitute", "--files", "*.md", "--sub", "no-separator").Code,
		"a malformed sub spec")
	assert.Equal(t, 2, r.Command("autosubstitute", "extra", "--files", "*.md", "--sub", "a=>b").Code,
		"packages are flags, not positional arguments")
}

// TestAutoSubstituteLeavesANestedPackageToItsOwner: a package folder holding
...
// goroutines.
func TestAutoSubstituteLeavesANestedPackageToItsOwner(t *testing.T) {
	r := harness.New(t)
	cfg := libsConfig(echoBuild, 2)
	...
	res := r.Command("autosubstitute", "--since", "all",
		"--files", "note.md", "--files", "**/note.md",
		"--sub", "pinned 0.0.0=>written by {name}")
	require.Equal(t, 0, res.Code, "stderr:\n%s", res.Stderr)
	assert.Contains(t, asRead(t, r, "packages", "outer", "note.md"), "written by outer")
	assert.Contains(t, asRead(t, r, "packages", "outer", "inner", "note.md"), "written by inner",
		"the nested package's own turn reached its file, and outer left it alone")
}

// TestAutoSubstituteCommandWordKeepsItsScript: like every command word,
// "autosubstitute" shadows a run script of the same name.
func TestAutoSubstituteCommandWordKeepsItsScript(t *testing.T) {
	...
	res := r.Command("run", "autosubstitute")
	require.Equal(t, 0, res.Code, "stderr:\n%s", res.Stderr)
	assert.Contains(t, res.Stdout, "the script ran")

	assert.Equal(t, 2, r.Command("autosubstitute").Code)
}
