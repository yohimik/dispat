...
// TestCovTailConfigReadsTheScalarShorthands: several keys take a list and are
// nearly always written as one value, and a section is nearly always written
// as the name of a built-in. Reading the short form is what keeps a
// hand-written config from being a list of one-element lists — and an element
// holding nothing at all is read as the empty thing it is and refused by the
// validation, rather than decoded into whatever happened to be next.
func TestCovTailConfigReadsTheScalarShorthands(t *testing.T) {
	// The shapes the typed model cannot express: a scalar where the model has
	// a list, and a section written as a built-in's name alone.
	base := func() map[string]any {
		return map[string]any{
			...
			"changelog": map[string]any{
				"sections": []any{"features", "fixes"},
				"header":   "one line above every entry",
			},
			"spaces": map[string]any{
				"libs": map[string]any{
					// The scalar spelling of a one-folder space.
					"path": "packages",
	...
	t.Run("the short spellings reach the same places the long ones do", func(t *testing.T) {
		r := harness.New(t)
		r.WriteConfigRaw(base())
		r.SeedPackage("packages", "core")
		r.Commit("feat(core): bootstrap")

		res := r.ReleaseOK()
		assert.True(t, r.IsTagged("core@0.1.0"),
			"the scalar path reached the same space the list spelling would; stdout:\n%s", res.Stdout)
		entry := covTailReadFile(t, r, "packages", "core", "CHANGELOG.md")
		assert.Contains(t, entry, "one line above every entry",
			"the scalar header is one line, not a config nobody read")
	})

	t.Run("an element holding nothing is refused rather than guessed at", func(t *testing.T) {
		for name, adjust := range map[string]func(map[string]any){
			"a section": func(cfg map[string]any) {
				cfg["changelog"].(map[string]any)["sections"] = []any{nil}
			},
			"an entry line": func(cfg map[string]any) {
				cfg["changelog"].(map[string]any)["footer"] = []any{nil}
			},
		} {
			t.Run(name, func(t *testing.T) {
				r := harness.New(t)
				...
				assert.Contains(t, res.Stdout+res.Stderr, "changelog:",
					"the refusal names the key the empty element sat under")
			})
		}
	})
}
