...
// TestHooksBeforeAllFailureEndsThePackageBeforeItsFirstStage: beforeAll is
...
func TestHooksBeforeAllFailureEndsThePackageBeforeItsFirstStage(t *testing.T) {
	r := harness.New(t)
...
// TestHooksVersionScriptsSkippedWhenEveryProviderDied: a consumer's version
...
func TestHooksVersionScriptsSkippedWhenEveryProviderDied(t *testing.T) {
...
// TestHooksStageOutputTruncatesAnOverlongLine: the head of the line is logged
...
func TestHooksStageOutputTruncatesAnOverlongLine(t *testing.T) {
	...
	t.Run("the rest of the line is dropped and the stage carries on", func(t *testing.T) {
		r := singlePackageRepo(t, megabyteOfX("x")+
	...
	t.Run("a line that never ends is dropped on the flush", func(t *testing.T) {
		r := singlePackageRepo(t, megabyteOfX("y"))
		r.Commit("feat(core): bootstrap")
...
// megabyteOfX writes a megabyte and a bit of one character to standard
...
func megabyteOfX(char string) string {
