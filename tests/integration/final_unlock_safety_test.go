...
// TestFinalFleetUnlockFailureFailsAfterPublishing proves that a fleet never
// reports success when one repository's remote release lock could not be
// returned. postAll runs after publication and all durable fleet records, so a
// source remote that starts refusing pushes there reaches precisely the final
...
func TestFinalFleetUnlockFailureFailsAfterPublishing(t *testing.T) {
	...
	hook := filepath.Join(fleet.sourceRemote, "hooks", "pre-receive")
	scripts["refuse-source-pushes"] = []string{
		`printf '#!/bin/sh\nexit 1\n' > ` + harness.ShQuote(hook) + ` && chmod +x ` + harness.ShQuote(hook),
	}
	cfg["run"] = map[string]any{"postAll": []string{"refuse-source-pushes"}}
	...
	assert.Contains(t, combined, "could not remove the release lock tag from the remote")
