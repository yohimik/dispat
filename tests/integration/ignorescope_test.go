...
// TestIgnoreScopeBareNameReachesAnyDepth: a pattern with no separator names the
// last segment of a path as well as the whole path, so one line covers the
// file wherever it sits.
func TestIgnoreScopeBareNameReachesAnyDepth(t *testing.T) {
...
// TestIgnoreScopeNestedFolderPatternCoversEverythingUnderIt: a folder named with
// a path, rather than by its bare name, excludes that one folder and
// everything below it while its siblings keep counting.
func TestIgnoreScopeNestedFolderPatternCoversEverythingUnderIt(t *testing.T) {
...
// TestIgnoreScopeEscapedBangNamesALiteralCharacter: a pattern beginning "\!"
...
func TestIgnoreScopeEscapedBangNamesALiteralCharacter(t *testing.T) {
...
// TestIgnoreScopeCommentsAloneSayNothing: a list of nothing but comments and
// blank lines compiles to no rules at all, so the level is dropped and every
// file of the package keeps counting.
func TestIgnoreScopeCommentsAloneSayNothing(t *testing.T) {
