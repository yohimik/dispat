...
// TestCovTailPropagationKindsSelectTheEdgesTraversed: which manifest fields
// imply "must be republished" is a fact about the repository, so §8.4 puts the
// choice in configuration rather than in any one commit. "*" traverses every
// field, and a list naming devDependencies traverses the one field the default
// deliberately leaves out.
func TestCovTailPropagationKindsSelectTheEdgesTraversed(t *testing.T) {
	kinds := func(values ...string) models.File {
		cfg := libsConfig(echoBuild, 1)
		cfg.Dependencies = []models.DependencyConfig{{Consumer: "near", Provider: "core"}}
		cfg.Parser = &models.ParserConfig{
			Propagation: &models.ParserPropagationConfig{Kinds: values},
		}
		return cfg
	}
	for _, row := range []struct {
		...
		{"every kind", []string{"*"}},
		{"the field the default leaves out", []string{"devDependencies"}},
	} {
		t.Run(row.name, func(t *testing.T) {
			r := harness.New(t)
			r.WriteConfigModel(kinds(row.kinds...))
			r.SeedPackage("packages", "core")
			r.SeedPackage("packages", "near")
			r.Commit("chore: seed the fleet")
			r.WriteFile("packages/core/work.txt", "work\n")
			r.Commit("feat(core)^: work that propagates one level")
