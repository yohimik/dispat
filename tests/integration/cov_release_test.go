...
// TestReleaseRefusesAMalformedExport: an export line that is not NAME=value is
// a typo that would otherwise silently drop the export, so it is an error of
// its own. Before the point of no return it fails the package; after it, where
// nothing can be taken back, it is a warning and the release stands.
func TestReleaseRefusesAMalformedExport(t *testing.T) {
	bad := `echo 'this line exports nothing' >> "$DISPAT_OUTPUT"`

	t.Run("a build that exports nonsense fails its package", func(t *testing.T) {
		r := singlePackageRepo(t, bad)
		r.Commit("feat(core): bootstrap")

		res := r.Release()
		assert.NotEqual(t, 0, res.Code, "stdout:\n%s", res.Stdout)
		assert.Contains(t, res.Stdout, "DISPAT_OUTPUT line 1")
		assert.Equal(t, 0, r.TagCount("core@"), "nothing released; tags: %v", r.TagList())
	})

	t.Run("an announce hook that does is only a warning", func(t *testing.T) {
		r := harness.New(t)
		cfg := libsConfig(echoBuild, 1)
		cfg.Scripts["announce"] = models.Script{bad}
		cfg.Spaces["libs"] = models.SpaceConfig{
			Path: models.PathList{"packages"},
		...
		res := r.ReleaseOK()
		assert.Contains(t, res.Stdout, "script outputs invalid")
