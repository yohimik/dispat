...
// Goal 52: the preflight a composed release runs over each
...
// simpleFleet is one source checked out under a control repository,
// with nothing configured beyond the link itself.
func simpleFleet(t *testing.T) *harness.Repo {
	t.Helper()
...
// TestPolyrepoPreflightRefusesARepositoryItCannotRecordInto: each
...
func TestPolyrepoPreflightRefusesARepositoryItCannotRecordInto(t *testing.T) {
	t.Run("work already sitting in a release path", func(t *testing.T) {
		control := simpleFleet(t)
...
// TestPolyrepoRefusesARecordPathThatIsNotAPath: the paths a release writes
...
func TestPolyrepoRefusesARecordPathThatIsNotAPath(t *testing.T) {
	t.Run("a changelog under a file", func(t *testing.T) {
		...
		res := control.Release()
		assert.Equal(t, 1, res.Code)
		assert.Contains(t, combinedOutput(res), "not a directory")
		assert.Empty(t, polyrepoTags(control, "sources/lib"))
	})
...
// TestPolyrepoIncludePathCannotChangeOwnerMidRun: preflight resolves every
...
func TestPolyrepoIncludePathCannotChangeOwnerMidRun(t *testing.T) {
	source := harness.New(t)
	source.SeedPackage("packages", "lib")
...
// TestPolyrepoExportThatIsNotACommitIsNotAdmitted: a native record step
...
func TestPolyrepoExportThatIsNotACommitIsNotAdmitted(t *testing.T) {
	source := harness.New(t)
	source.SeedPackage("packages", "lib")
	source.Commit("feat(lib): bootstrap library")
