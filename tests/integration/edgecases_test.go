...
//     rests on;
//   - a shared `commit.include` lock file a package wrote into before it
//     failed or was skipped, which the release commit must not record.
//
...
// sharedLockRegenerator is the syncLock script of the shared lock-file
// scenarios: a whole-workspace regenerator in the manner of `pnpm install`,
// writing every workspace manifest's version into lock.txt at the repository
// root, in path order. It leaves `<package>-synced` behind when it finishes.
const sharedLockRegenerator = `cd ../.. && for f in */*/package.json; do ` +
	`printf '%s %s\n' "${f%/package.json}" "$(sed -n 's/.*"version": *"\([^"]*\)".*/\1/p' "$f")"; ` +
	`done > lock.txt && : > "$DISPAT_PACKAGE-synced"`

// sharedLockRepo is a single history in commit mode whose root lock.txt is a
// `commit.include` path regenerated by every `libs` package's syncLock. util
// publishes; every other `libs` package holds its version stage until util's
// lock sync has finished, so that its own sync is the last write the run makes
// to lock.txt. The caller adds the packages and the scenario's scripts, and
// commits.
func sharedLockRepo(t *testing.T, edit func(*models.File)) *harness.Repo {
	t.Helper()
	r := harness.New(t)
	...
	cfg.Scripts = map[string]models.Script{
		...
		"after-util": {`if [ "$DISPAT_PACKAGE" != util ]; then ` +
			`while [ ! -e ../../util-synced ]; do sleep 0.05; done; fi`},
	}
	cfg.Spaces = map[string]models.SpaceConfig{
		"libs": {Path: models.PathList{"packages"}, Flow: &models.SpaceFlowConfig{
			BeforeVersion: []string{"after-util"}, Build: []string{"build"}, Publish: []string{"publish"}},
...
// seedManifest writes a package's main file and its package.json at 0.0.0.
func seedManifest(r *harness.Repo, space, name string) {
	r.SeedPackage(space, name)
	r.WriteFile(space+"/"+name+"/package.json", `{"name": "`+name+`", "version": "0.0.0"}`)
}

// TestEdgeReleaseCommitLockOmitsASkippedConsumer: a consumer whose version
// stage and lock sync ran is skipped when its provider fails to publish, while
// an independent package publishes. The consumer's sync wrote its planned
// version into the shared lock file last, so the closing phase restores the
// file and runs the published package's syncLock again with the consumer at
// its previous version: the release commit's lock.txt lists only versions
// that published.
func TestEdgeReleaseCommitLockOmitsASkippedConsumer(t *testing.T) {
	r := sharedLockRepo(t, func(cfg *models.File) {
		// core fails only once web's lock sync has written its planned
		// version, so that the skip comes after the write it must undo.
		cfg.Scripts["fail"] = models.Script{"while [ ! -e ../../web-synced ]; do sleep 0.05; done; exit 1"}
		cfg.Spaces["core"] = models.SpaceConfig{Path: models.PathList{"providers"},
			Flow: &models.SpaceFlowConfig{Build: []string{"build"}, Publish: []string{"fail"}}}
		cfg.Dependencies = []models.DependencyConfig{{Consumer: "web", Provider: "core"}}
	})
	seedManifest(r, "providers", "core")
	seedManifest(r, "packages", "util")
	seedManifest(r, "packages", "web")
	r.WriteFile("lock.txt", "packages/util 0.0.0\npackages/web 0.0.0\nproviders/core 0.0.0\n")
	r.Commit("feat(core)^: reaches web, but fails to publish")
	r.WriteFile("packages/util/main.txt", "util\nchanged\n")
	r.Commit("feat(util): an independent change")

	res := r.Release()
	require.Equal(t, 1, res.Code, "stdout:\n%s", res.Stdout)
	require.True(t, harness.IsCodePresentForPackage(res.Events, "W194", "web"), "web is skipped behind core")
	require.True(t, r.IsTagged("util@0.1.0"), "tags: %v", r.TagList())
	assert.Zero(t, r.TagCount("web@"))
	assert.Equal(t, "packages/util 0.1.0\npackages/web 0.0.0\nproviders/core 0.0.0\n",
		r.Git("show", "HEAD:lock.txt")+"\n", "the release commit records only published versions")
	assert.Contains(t, r.Git("log", "-1", "--format=%s"), "util@0.1.0", "HEAD is the release commit")
	assert.Empty(t, r.Git("status", "--porcelain", "--", "lock.txt", "packages"),
		"the shared file and the package folders match the release commit")
}

// TestEdgeReleaseCommitLockOmitsAFailedWriter: a package whose version stage
// and lock sync ran fails its build. Its tracked files are restored before the
// published package's syncLock runs again, so the regenerated lock file reads
// its manifest at the version that exists, while the untracked output its
// build left stays for inspection.
func TestEdgeReleaseCommitLockOmitsAFailedWriter(t *testing.T) {
	r := sharedLockRepo(t, func(cfg *models.File) {
		cfg.Scripts["build"] = models.Script{`if [ "$DISPAT_PACKAGE" = core ]; then echo partial > out.txt; exit 1; fi`}
	})
	...
	assert.Equal(t, "packages/core 0.0.0\npackages/util 0.1.0\n", r.Git("show", "HEAD:lock.txt")+"\n",
		"the release commit records only published versions")
	assert.Contains(t, readFileString(t, r.Path("packages", "core", "package.json")), `"version": "0.0.0"`,
		"the failed package's tracked files are restored")
...
// it out and still records the published package, and E223 names the remedy,
// which regenerates the file from what published.
func TestEdgeReleaseCommitLockLeftAtHeadWhenResyncIsInterrupted(t *testing.T) {
	r := sharedLockRepo(t, func(cfg *models.File) {
		cfg.Scripts["build"] = models.Script{`if [ "$DISPAT_PACKAGE" = core ]; then exit 1; fi`}
		// util's second sync, the closing phase's, writes half a file and
		// hangs until the interrupt; any later sync regenerates normally.
		cfg.Scripts["regen"] = models.Script{`if [ "$DISPAT_PACKAGE" = util ] && [ -e ../../util-synced ] && ` +
			`[ ! -e ../../resync-started ]; then : > ../../resync-started; echo partial > ../../lock.txt; sleep 30; fi`,
			sharedLockRegenerator}
	})
	seedManifest(r, "packages", "core")
	seedManifest(r, "packages", "util")
	...
	require.Eventually(t, func() bool {
		_, err := os.Stat(r.Path("resync-started"))
		return err == nil
	}, 60*time.Second, 20*time.Millisecond, "the closing phase never ran util's syncLock again")
	...
	require.True(t, r.IsTagged("util@0.1.0"), "the published package is still tagged; tags: %v", r.TagList())
	assert.Contains(t, r.Git("log", "-1", "--format=%s"), "util@0.1.0", "the release commit is still made")
	assert.Equal(t, seed, r.Git("show", "HEAD:lock.txt")+"\n", "the release commit leaves the shared file out")
	assert.Equal(t, seed, readFileString(t, r.Path("lock.txt")), "the half-written file is restored")

	remedy := r.Command("run", "regen", "--package", "util", "--since", "all")
	require.Equal(t, 0, remedy.Code, "stdout:\n%s\nstderr:\n%s", remedy.Stdout, remedy.Stderr)
