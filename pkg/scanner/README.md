...
Both methods share one error contract: a manifest that fails to parse is skipped, its error joined into the
returned error, and the parsed manifests come back either way, so callers can report the problem and keep
the partial result. Reads are capped at 16 MiB per file (`ErrManifestTooLarge`); output order is
deterministic.
...
Helpers shared by the CLI's two consumers: `NameIndex` (manifest name → owning package, root manifests
...
(`Directory.Packages.props`). Version text is always kept verbatim, so name matching still carries the
graph where the version is indirected.
