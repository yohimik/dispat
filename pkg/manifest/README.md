...
[`pkg/writer`](../writer) (the manifest rewriter). Both packages use its format list, dependency kinds, and Docker reference parser.
Conformance tests check that their supported formats stay aligned.

The surface, by concern:

- **Formats.** `Format` names each of the thirty-six recognised manifest formats, `Formats` is the canonical list both
...
- **Docker.** `ParseImageRef` reads an image reference into repository, tag, and digest with the tag's byte span, while
  `IsValidTag` checks registry tag grammar. `DockerfileRefs` walks a Dockerfile's `FROM`, `COPY --from`, and
...
Boolean predicates use `Is` names, including `Kind.IsValid()` and `ImageRef.IsPinned()`. Earlier exported predicate
names remain as deprecated forwarding functions for source compatibility.
