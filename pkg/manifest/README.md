...
(the manifest rewriter): the dependency-field kinds a manifest declares, the file-name rules that decide
what counts as a manifest, and the name normalisation both sides must apply identically. It exists so the
reading and writing halves of dispat's manifest support can never drift apart.
