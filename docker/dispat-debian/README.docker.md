...
The [dispat](https://github.com/yohimik/dispat) release orchestrator CLI on `debian:trixie-slim`, for pipelines whose
tooling expects a Debian userland, for `linux/amd64` and `linux/arm64`.
...
`yohimik/dispat-alpine` (the smallest), `yohimik/dispat-ubuntu` (the default), and `yohimik/dispat-dind` for release
flows whose build stage is itself a `docker build`.
