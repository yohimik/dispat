...
The smallest [dispat](https://github.com/yohimik/dispat) image: the release orchestrator CLI on `alpine:3.24`, about
15 MB, for `linux/amd64` and `linux/arm64`.
...
`yohimik/dispat-ubuntu` (the default, most CI runners' own base), `yohimik/dispat-debian`, and `yohimik/dispat-dind`
for release flows whose build stage is itself a `docker build`.
