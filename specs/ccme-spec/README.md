...
- [Choreographed saga](./SPEC.md#2711-choreographed-saga): the same profile without a control repository. Peers carry their own identity, configuration and records, two-sided fleet links join them, a release may start in any of them, and a cross-repository boundary is proven from the links a release recorded rather than from a checkpoint.
