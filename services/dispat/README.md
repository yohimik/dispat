...
- **The graph writes itself.** `dispat compute` derives consumer/provider edges from the packages' own manifests (npm,
  Go, Cargo, Python, Composer, Maven, .NET, Dart) and keeps the config honest with `--check` in CI; an
  `autoVersion` space has dispat rewrite its manifests natively at the version stage (ranges reconciled, own versions
  written, lock files regenerated by `syncLock`), format-preservingly and byte-precise.
- **Release records built in.** Per-package changelogs, annotated tags, GitHub releases, an optional single release
  commit plus push; all customisable or disableable, globally or per package.
...
Together they hold **93.3%** workspace statement coverage
