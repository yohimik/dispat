...
- **The graph writes itself.** `dispat compute` derives consumer/provider edges from the packages' own manifests
  (npm, Go, Cargo, Python, Composer, Maven, .NET, Dart) and keeps the config honest with `--check` in CI; an
