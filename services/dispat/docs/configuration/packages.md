...
the repository top — anything that releases like a package but shares no parent folder with one. The path is relative to
...
needs another kind or [`keep`](./README.md#top-level-options) is declared in the top-level
...
A package folder may carry a dispat config file of its own — the same names and formats the root config resolves through
(`dispat.json`, `dispat.yaml`, `dispat.yml`, `dispat.toml`, first match wins). Its top-level object is exactly the
