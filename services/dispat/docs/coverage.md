...
per-layer numbers (unit: all modules, **84.4%**; integration: the CLI module through the instrumented binary, **84.0%**)
...
The unit layer covers every package against in-memory fakes; on its own it reaches ~84%, because the composition (the
