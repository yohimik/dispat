...
// The download command's decisions, short of actually installing a binary:
...
// dlOpts is one invocation, with somewhere to collect what it printed and a
// folder nothing else is using.
func dlOpts(t *testing.T, src selfupdate.Source) (*DownloadOptions, *bytes.Buffer) {
	t.Helper()
	var out bytes.Buffer
	return &DownloadOptions{
		Repository: download.Repository{Owner: "acme", Repo: "tool"},
...
// TestDownloadCheckAnswersWhetherThereIsAnythingToDo: the whole of what --check
...
func TestDownloadCheckAnswersWhetherThereIsAnythingToDo(t *testing.T) {
	opts, out := dlOpts(t, toolReleases(t, "v1.4.0", "tool-linux-amd64"))
	opts.Check = true

	pending, err := Download(context.Background(), *opts)
	...
	assert.Contains(t, out.String(), "install it with: dispat download")
...
// TestDownloadCheckReportsAPipeInstead: a pipe has no destination file to
...
func TestDownloadCheckReportsAPipeInstead(t *testing.T) {
	opts, out := dlOpts(t, toolReleases(t, "v1.4.0", "tool-linux-amd64"))
...
// TestDownloadCheckSpeaksJSON: the report is for a person and the event is for
// the stream CI already ingests, and the two say the same thing.
func TestDownloadCheckSpeaksJSON(t *testing.T) {
	opts, out := dlOpts(t, toolReleases(t, "v1.4.0", "tool-linux-amd64"))
	...
	require.NoError(t, err)
	assert.True(t, pending)
	event := jsonEvent(t, out.String(), "download check")
	...
	opts.Check = false
	out.Reset()
	_, err = Download(context.Background(), *opts)
	require.NoError(t, err)
...
// TestDownloadRefusesBeforeItFetchesAnything: every one of these is decided
// before a single byte of the asset moves, and each says what to do about it.
func TestDownloadRefusesBeforeItFetchesAnything(t *testing.T) {
	for name, tc := range map[string]struct {
		...
		"a release nobody published": {
			mutate: func(o *DownloadOptions) { o.Release = "9.9.9" },
		...
		"a placeholder nobody defines": {
			mutate: func(o *DownloadOptions) { o.Asset = "tool-{arch64}" },
			want:   "{arch64}",
		},
		"a name that is a path": {
			mutate: func(o *DownloadOptions) { o.Name = "../evil" },
			want:   "not a path",
		},
		"nowhere to install it": {
			mutate: func(o *DownloadOptions) {
				o.BinDir = ""
				o.Env = emptyEnv{}
			},
			want: "--bin-dir",
...
// TestDownloadRefusesADestinationItMustNotReplace: a name that is already a
...
func TestDownloadRefusesADestinationItMustNotReplace(t *testing.T) {
	opts, _ := dlOpts(t, toolReleases(t, "v1.4.0", "tool-linux-amd64"))
...
// TestDownloadWithoutAChecksumSaysSo: a release that publishes none cannot be
...
func TestDownloadWithoutAChecksumSaysSo(t *testing.T) {
	src := toolReleases(t, "v1.4.0", "tool-linux-amd64")
	src.Command = download.Command
	opts, out := dlOpts(t, src)
	opts.Check = true
...
// TestDownloadRollbackNeedsNoRelease: a rollback restores what is already
// installed, so it reads nothing and reports what it found either way.
func TestDownloadRollbackNeedsNoRelease(t *testing.T) {
	opts, out := dlOpts(t, selfupdate.Source{APIURL: "http://example.invalid"})
...
// TestDownloadRollbackSpeaksJSON: the same three answers as fields, for the
// job that has to record what it did to a runner.
func TestDownloadRollbackSpeaksJSON(t *testing.T) {
	opts, out := dlOpts(t, selfupdate.Source{APIURL: "http://example.invalid"})
...
// TestDownloadReportsADestinationItCannotRead: a file dispat may replace but
...
// past.
func TestDownloadReportsADestinationItCannotRead(t *testing.T) {
	if runtime.GOOS == "windows" || os.Geteuid() == 0 {
		t.Skip("the scenario needs a file the running user cannot read")
	}
	opts, _ := dlOpts(t, toolReleases(t, "v1.4.0", "tool-linux-amd64"))
	opts.Check = true
	require.NoError(t, os.WriteFile(filepath.Join(opts.BinDir, "tool"), []byte(dlBody), 0o000))
...
// TestDownloadReportsAFolderItCannotCreate: the install folder is created on
// the way, and a parent that refuses is a refusal rather than a download
// nobody can place.
func TestDownloadReportsAFolderItCannotCreate(t *testing.T) {
	if runtime.GOOS == "windows" || os.Geteuid() == 0 {
...
// TestDownloadNamesBothFlagsThatWouldFindARelease: a repository whose releases
...
func TestDownloadNamesBothFlagsThatWouldFindARelease(t *testing.T) {
	opts, _ := dlOpts(t, toolReleases(t, "not-a-version"))
	opts.Check = true
...
// TestDownloadSaysWhenTheFolderIsNotOnPath: a tool the shell cannot find is a
// successful install that looks like a failed one, so the one thing that makes
// it look that way is said out loud.
func TestDownloadSaysWhenTheFolderIsNotOnPath(t *testing.T) {
	dir := t.TempDir()
...
// TestDownloadOptionsFallBackToTheRealMachine: the environment is a field so a
// test can ask for a machine it is not running on, and nil is the real one.
func TestDownloadOptionsFallBackToTheRealMachine(t *testing.T) {
	opts := DownloadOptions{GOOS: "plan9"}
	assert.Equal(t, "plan9", opts.env().GOOS())
	assert.False(t, opts.piping())

	var out, errs bytes.Buffer
	opts = DownloadOptions{Out: &out, Err: &errs, Pipe: "sh"}
...
func writeDigestFile(t *testing.T, path string) {
	t.Helper()
	require.NoError(t, os.WriteFile(path, []byte(dlBody), 0o755))
}
