...
func validConfig() File {
	return File{
		...
		Spaces: map[string]SpaceConfig{
			"libs": {Path: PathList{"packages/libs"}, IsBuildWaitingPublish: models.StageRelationOf(true), RevertOnFail: models.Bool(true),
...
func TestDiscover(t *testing.T) {
	...
	cfg, err := Load(filepath.Join(root, "dispat.json"), nil)
	require.NoError(t, err)
	pkgs, deps, _, err := DiscoverWorkspace(cfg, root, nil)
	require.NoError(t, err)
...
func TestDiscoverDuplicatePackage(t *testing.T) {
	...
	cfg, err := Load(filepath.Join(root, "dispat.json"), nil)
	require.NoError(t, err)
	_, _, _, err = DiscoverWorkspace(cfg, root, nil)
	require.Error(t, err)
...
func TestLoadScriptArraysAndScalars(t *testing.T) {
	...
	cfg, err := Load(filepath.Join(root, "dispat.json"), nil)
	require.NoError(t, err)
	pkgs, _, _, err := DiscoverWorkspace(cfg, root, nil)
	require.NoError(t, err)
	require.Len(t, pkgs, 1)
...
func TestSyncLockResolvesThroughThePackageScope(t *testing.T) {
	...
	require.NoError(t, err)
	_, _, _, err = DiscoverWorkspace(loaded, badRoot, nil)
	require.Error(t, err)
...
func TestDiscoverMissingExternalProvider(t *testing.T) {
	...
	loaded, err := Load(filepath.Join(root, "dispat.json"), nil)
	require.NoError(t, err)
	_, deps, _, err := DiscoverWorkspace(loaded, root, nil)
	require.NoError(t, err)
	assert.Empty(t, deps)
