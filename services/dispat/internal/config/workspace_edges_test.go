...
func TestComposeWorkspaceRejectsInvalidControlAndImportRoots(t *testing.T) {
	t.Run("missing control root", func(t *testing.T) {
		...
		workspace, err := ComposeWorkspaceWithPinResolver(context.Background(), &File{Polyrepo: true, Packages: map[string]PackageConfig{"app": {Path: "sources/sdk/pkgs/sdk"}}}, filepath.Join(missing, "dispat.json"), missing, nil, nil, nil)
		require.Nil(t, workspace)
	...
	t.Run("control root is not Git", func(t *testing.T) {
		...
		workspace, err := ComposeWorkspaceWithPinResolver(context.Background(), &File{Polyrepo: true, Packages: map[string]PackageConfig{"app": {Path: "sources/sdk/pkgs/sdk"}}}, path, root, nil, nil, nil)
		require.Nil(t, workspace)
