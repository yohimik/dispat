...
func TestParseTagsForPackagesMatchesIndependentPerPackageSemantics(t *testing.T) {
	...
	assert.Empty(t, got["core"][2].Commit, "an explicit empty object field retains the old parser semantics")
	assert.Equal(t, "core@2.2.0", got["core"][3].Name, "field padding is trimmed without changing tab structure")
	assert.Len(t, got["wide"], 7, "a broad custom format receives every non-reserved well-formed ref")
}

func TestMatchedTagsDetachFromRawInventory(t *testing.T) {
	const commit = "1111111111111111111111111111111111111111"
	raw := strings.Repeat("unmatched-padding", 1<<16) + "\ncore@1.2.3\t" + commit + "\t\n"

	bulk := parseTagsForPackages(raw, map[string]TagFormat{
		"core": DefaultTagFormat,
		"wide": "{version}",
	})
	...
	single := parseTags(raw, "core", DefaultTagFormat)
...
func referenceTagsForPackage(out, pkg string, format TagFormat) Tags {
	...
	for _, line := range strings.Split(out, "\n") {
		...
		if name == LockTagName || strings.HasPrefix(name, LockAttemptTagPrefix) || !format.Matches(pkg, name) {
			continue
		}
...
func BenchmarkParseTagsForPackagesIndexed(b *testing.B) {
	...
	for _, tc := range []struct {
		...
		b.Run(tc.name+"/indexed", func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				benchmarkTagResults = parseTagsForPackages(tc.raw, tc.formats)
			}
		})
		b.Run(tc.name+"/per_package_scan", func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				...
				for pkg, format := range tc.formats {
					result[pkg] = parseTags(tc.raw, pkg, format)
				}
