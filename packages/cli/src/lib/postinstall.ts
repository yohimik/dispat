...
export async function main(options: PostinstallOptions = {}): Promise<number> {
  ...
  try {
    ...
    stderr.write('dispat: after fixing the problem, run `node node_modules/@dispat/cli/build/bin/postinstall.js` locally; for a global install run the same file under the directory printed by `npm root -g`.\n')
