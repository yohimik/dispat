...
// Keep Docusaurus 3.10.2 category behavior, but use its separate caret button
// for every collapsible category, including groups without an overview page.
...
function DocSidebarItemCategoryCollapsible({
  ...
  // The unlinked label is exposed as a button after hydration. Anchors
  // already activate with Enter; add the button convention for Space.
  const handleItemKeyDown: ComponentProps<'a'>['onKeyDown'] = (e) => {
    if (collapsible && !href && e.key === ' ') {
      e.preventDefault();
      updateCollapsed();
    }
  };
