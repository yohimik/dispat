...
phase, the commands, signal handling) count too. The profiles are merged block by block — a block compiled by two test
binaries is one block, covered if either reached it — and the merged total becomes the badge on the repository README.
...
would be a number about whichever packages happened to change — which is why the badge is computed by the release
workflow and not by the per-commit test workflow.
