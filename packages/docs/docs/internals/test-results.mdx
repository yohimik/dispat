...
`go test -json` stream for exactly this. `tests` and `fuzz` count test *functions*, never subtests — a suite that
counts every `t.Run` as a test reports a number several times the one it can defend, so the subtests are counted
beside them instead.

The integration suite appears twice: once instrumented for coverage, once under the race detector. CI runs it that way
too, rather than with `-count`. The totals above count it once — the race pass is the same tests again, and adding it
in would claim several hundred tests this repository does not have.
...
plan carries the claim-by-claim matrix — a row per test, saying which goal it proves; this is the summary.
