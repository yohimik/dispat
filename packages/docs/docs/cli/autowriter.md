...
| `--range`             | from config | `autoversion` and `autowriter`: override the [`autoVersion.range`](../configuration/spaces.md#autoversion) write policy, which is how a reconciled range is spelled (`caret`, `tilde`, `exact`, a `{version}` template, or a literal). For `autowriter` it spells what `--set-local` derives. |
