...
| `--set-build`         |             | `writer` only: write the manifest's build counter where its format keeps one — `CFBundleVersion`, `android:versionCode`, `CURRENT_PROJECT_VERSION`, Gradle's `versionCode`, a pubspec version's `+` suffix. A counter the file does not declare is not created (the pubspec suffix, part of the version scalar, is the one exception); Android and Gradle counters must be integers. |
