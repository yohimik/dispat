...
| `--range`             | from config | `autoversion` and `autowriter`: override the [`autoVersion.range`](../configuration/autoversion.md) write policy, which is how a reconciled range is spelled (`caret`, `tilde`, `exact`, a `{version}` template, or a literal). |
