...
[Substituting text across the monorepo](../editing/autosubstitute.md).
...
| `--sub`               |             | `replacer` and `autosubstitute`: replace literal text, `find=>write`; repeatable and applied in order. See [The replacer](../editing/replacer.md). |
