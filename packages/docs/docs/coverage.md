...
The whole test suite currently covers **94.9%** of the workspace's statements (unit layer alone: 89.4%; the integration
layer's instrumented binary: 82.5%).
...
**2026-08-11** (after the `--package` / `--space` selection landed) with Go 1.26 using the steps under [Reproducing](#reproducing), and drifts until someone regenerates it.

| Module / package                        | Statement coverage                                                                       |
|-----------------------------------------|------------------------------------------------------------------------------------------|
| **workspace total**                     | **94.9%**                                                                                |
...
| `services/dispat` (all packages)        | **95.1%** aggregate, `main.go` included (the integration suite runs the real binary)     |
