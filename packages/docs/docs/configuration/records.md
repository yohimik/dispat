...
| `include`       | none                     | Extra repo-relative paths the release commit stages on top of the published packages' folders. This includes the shared artifacts a version stage or an [`autoVersion.syncLock`](./autoversion.md) regenerates outside every package folder, a workspace-level lock file (`package-lock.json`, `pnpm-lock.yaml`, `yarn.lock`) first among them. Paths must stay inside the repository. One that does not exist at commit time is simply not staged. When a package whose version or `syncLock` stage ran does not publish, a single repository re-synchronizes these paths before the release commit so that it records only published versions; see [Shared files of the release commit](../reference/releasing/recovery.md#shared-files-of-the-release-commit). |
