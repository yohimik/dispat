...
A space may be rooted at the repository itself with `path: .` (or `path: ./`), in which case its packages are the
repository's top-level folders. The same rules decide what a package is there: hidden folders are skipped, and a
[`.dispatexclude`](#dispatexclude) at the top of the repository names the folders that are not packages. Nothing may
be listed beside the root, because every other folder is already inside it and no folder may belong to one space
twice. A space rooted there is not a narrowing either: running a command at the top of the repository still covers
every package.
...
| `revertOnFail`          | bool                     | no (false) | When `true`, dispat rolls back all local changes inside the package folder if the package fails at any stage, or is skipped after its version stage already modified files. Tracked files are restored from HEAD, and untracked files are removed. With [release commits](./records.md#commit) enabled, a skipped package's folder is restored this way whether or not the setting is on, unless the folder is the repository root or holds another package's folder. A failed package keeps this setting's rule, except that its tracked files are restored when the release commit's shared `commit.include` paths are re-synchronized. See [What a failed run leaves in the working tree](../reference/releasing/recovery.md#what-a-failed-run-leaves-in-the-working-tree). |
...
| `buildOutputs`          | array of strings         | no         | The folders this space's builds leave behind, as literal slash-separated paths relative to each package folder, ignored files included. They are what [distributed execution](../distributed-execution.md#build-outputs-as-inputs) captures from the node that built them and installs on the nodes that consume them. A level replaces the list whole, and an explicit empty list opts out. A root is installed by replacing the folder it names, so it may not be the package folder itself, may not sit inside another root, and may not hold another package's folder. Nothing else about a release reads the key. |
| `buildPlatforms`        | array of strings         | no         | The node platforms this space's builds may run on, as `os/arch` in Go's spelling (`linux/amd64`). Empty or absent means any node. A platform no configured worker node reports refuses the run with `E225` before anything is dispatched, unless the package is pinned to the orchestrator with `runOnly`. |
| `runOnly`               | string or `[string, string]` | no     | Where this space's delegable stages may be placed: `both` (the default), `worker` or `orchestrator`, as one value for both stages or a `[build, publish]` pair. A space that configures `flow.login` publishes on the orchestrator whatever this says, and `publish: worker` beside a login is refused when the file loads. See [Where a stage runs](../distributed-execution.md#where-a-stage-runs). |
