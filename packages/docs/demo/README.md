...
| `Infra` | live scene | A versioned Terraform package rebuilds temporary state from known resources, saves a plan, and applies it while backend and frontend build independently. Both deployments wait for the apply. Git tags record completion without a separate progress database or persistent state bucket in this setup. | the carousel: "Terraform" |
...
The transcript keeps the reader's open or closed choice across slide changes. Captions and open transcripts reserve the tallest content at the current width, so their text column and the page below stay in place. Failure feedback overlays the player without shifting that text. Selecting a scene loads only that scene’s chunk; the current scene and its caption stay visible until the selection is ready. Scene loading displays no indicator or cancel button. Choosing another scene supersedes the pending selection; a late download cannot replace the newer choice. A failed load keeps the current scene available and offers a retry.
