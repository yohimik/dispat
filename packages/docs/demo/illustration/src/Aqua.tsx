...
export const Aqua: React.FC = () => {
  ...
  const compact = height <= 800 || mobile;
  ...
  return (
    <AbsoluteFill style={{background: colors.bg, color: colors.fg, fontFamily: font, padding: inset}}>
      ...
      <div style={{display: 'flex', flexDirection: mobile ? 'column' : 'row', gap: mobile ? 18 : 32, marginTop: compact ? 32 : 80}}>
        {fixture.selectedPackages.map((name, index) => {
          ...
          return (
            <div key={name} style={{flex: mobile ? 'none' : 1, minWidth: 0, minHeight: mobile ? 128 : compact ? 180 : undefined, opacity: show(frame, 32 + index * 26), border: `2px solid ${changed ? colors.green : colors.panelEdge}`, borderRadius: 18, background: colors.panel, padding: mobile ? '18px 24px' : compact ? '24px 28px' : 30}}>
